...
function InterviewSetupPage() {
    ...
    const loadResume = async () => {
      try {
        ...
        if (data.resumeText) {
          ...
          setResumeFileName(data.fileName || 'Previously uploaded resume');
        }
      } catch (error) {
        // No saved resume, that's fine
      }
    };
    loadResume();
  }, []);
  ...
  const handleStartInterview = async () => {
    ...
    setLoading(true);
    try {
      const data = await startInterview(selectedRole, selectedDifficulty, resumeText);
